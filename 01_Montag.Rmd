...
Für eine erste Annäherung an die Daten helfen uns drei Überblicksbefehle: `descrive`, `browse` und `list`.
...
Mit `inrange` können wir Auswahlen bequem auf einen Wertebereich eingrenzen - d.h. diese beiden Auswahlen führen zum gleichen Ergebnis:
```{stata browse_if, eval = F}
list sex land if age >= 20 & age <= 30
list sex land if inrange(age,20,30)
```
...
+ Wechseln Sie mit `cd` das Arbeitsverzeichnis!
...
+ Lesen Sie den Allbus 2018 in Stata ein.
  ...
  + Unter welchem Variablennamen ist die Selbsteinschätzung des Gesundheitszustands der Befragten ablegt?
  + Unter welchem Variablennamen ist die Information abgelegt, in welchem Bundesland die Befragten wohnen?
  + Welches Skalenniveau haben die beiden erwähnten Variablen? [^14] 
  ...
  + Wie alt ist der/die Befragte mit der `respid` 3469? Welches Geschlecht und welche Wahlabsicht hat die Person angegeben?
  
  
[^14]: Diese Frage ist eine inhaltliche, Stata hilft Ihnen hier nicht weiter.  

## Profi-Übungen

Für alle, die noch etwas mehr machen möchten: 

+ Auf ILIAS finden Sie auch eine csv-Datei mit aktuellen Daten zu Testzahlen, Fallzahlen und regionalen Gegebenheiten rund um COVID19 aus dem [tidycovid19](https://joachim-gassen.github.io/2020/05/tidycovid19-new-data-and-doc/)-Projekt. Importieren Sie diesen Datensatz per csv-Import! (Siehe [weitere Anmerkungen](#otherfmts))

+ Wie viele Zeilen und Spalten hat dieser Datensatz?
+ Welcher Variablentyp ist für die Variable `country` hinterlegt?
+ Löschen Sie alle Beobachtungen für die USA! Wie viele Fälle bleiben übrig?
