...
Mit durch Anhängen einer if-Bedingung können wir nur bestimmte Zeilen anzeigen lassen. Zum Beispiel nur die 20-jährigen Befragten
```{stata browse3, eval = F}
browse eastwest german sex age mstat if age == 20
```

Oder Befragte mit einem Einkommen unter 1000 Euro:
```{stata browse4, eval = F}
browse eastwest german sex age mstat if inc < 1000
```
