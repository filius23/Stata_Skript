...
```{stata twoway2, echo = F,results="hide",collectcode=TRUE, eval = F}
graph twoway (scatter inc age) (lfit inc age)
...
```{stata theme, echo = 1,results="hide",collectcode=TRUE, eval = F}
...
```{stata theme2, echo = 1,results="hide",collectcode=TRUE, eval = F}
graph twoway (scatter inc age) (lfit inc age), scheme(plotplain) ///
  legend(cols(2) pos(6))
...
```{stata theme3, echo = 1,results="hide",collectcode=TRUE, eval = F}
...
```{stata colored_scatter, echo=1, results="hide",collectcode=TRUE, eval = F}
...
```
```{stata colored_scatter_ex, echo=F, results="hide",collectcode=TRUE, eval = F}
...
```{stata colored_line_scatter, echo=1, results="hide",collectcode=TRUE, eval = F}
...
, scheme(lean2) legend(off) xtitle("")
```
```{stata colored_line_scatter_ex, echo=F, results="hide",collectcode=TRUE, eval = F}
...
```{stata loadbp, echo=T, results="hide",collectcode=TRUE}
webuse bpwide, clear
```

```{stata fancy_bpplot, eval=F, results="hide",collectcode=TRUE, eval = F}
...
```
```{stata fancy_bpplot_export, echo=F, results="hide",collectcode=TRUE, eval = F}
graph box bp_before bp_after, over(agegrp) box(1, color("0 0 128"))  scheme(lean2) ytitle("{stSerif:Blutdruck}{superscript:XX}") title("Blutdruckdaten ({it:fiktiv})") caption("{bf:Quelle}: Stata-Daten {it:bpwide}, ein fiktives Beispiel") 
