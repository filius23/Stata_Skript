...
```{stata theme, echo = 1,results="hide",collectcode=TRUE}
...
```{stata theme2, echo = 1,results="hide",collectcode=TRUE}
...
```{stata theme3, echo = 1,results="hide",collectcode=TRUE}
...
```{stata colored_scatter, echo=1, results="hide",collectcode=TRUE}
...
```
```{stata colored_scatter_ex, echo=F, results="hide",collectcode=TRUE}
...
```{stata colored_line_scatter, echo=1, results="hide",collectcode=TRUE}
...
, scheme(lean2) legend(off) xtitle("")
```
```{stata colored_line_scatter_ex, echo=F, results="hide",collectcode=TRUE}
...
```{stata fancy_bpplot, eval=F, results="hide",collectcode=TRUE}
...
```
```{stata fancy_bpplot_export, echo=F, results="hide",collectcode=TRUE}
graph box bp_before bp_after, over(agegrp) box(1, color("0 0 128"))  scheme(lean2) ytitle("{stSerif:Blutdruck}{superscript:XX}") title("Blutdruckdaten ({it:fiktiv})") caption("{bf:Quelle}: Stata-Daten {it:bpwide}, ein fiktives Beispiel") 
