...
Zusammenhänge sind das Herz (fast) aller statistischer Analysen. Im Folgenden lernen wir die Berechnung einer Reihe von Kennzahlen kennen, welche den Zusammenhang zwischen zwei Variablen ausdrücken. Diese variieren zwischen 0 und 1 oder -1 und +1, der Abstand zur Null dient dabei als Indikator für die Stärke des Zusammenhangs:
...
Regressionsmodelle zeigen uns den (linearen) Trend zwischen zwei Variablen. Hier geht es darum, um wie sich im Durchschnitt `var2` verändert, wenn sich `var1` um eine Einheit erhöht. Im Folgenden sehen wir, dass Regressionsmodelle mit `reg` erstellt werden können und wie wir den Output interpretieren können. Falls Sie nochmal mehr zu den Grundlagen erfahren wollten, findet sich [hier](#reg) eine ausführlichere Erklärung der Grundlagen. 
...
Außerdem erkennen wir unter `R-squared` die [Modellgüte](#r2) unseres Regressionsmodells. Das $R^2$ hilft uns einzuschätzen, um wieviel besser unsere Gerade aus `reg` die Werte vorhersagen kann als das einfache arithmetische Mittel. Das $R^{2}$ für unser `reg`-Modell gibt die prozentuale Verringerung der [Residuen](#resid2) durch das `reg`-Modell im Vergleich zur Mittelwertregel an: 
...
Die vorhergesagten Werte aus `reg var2 var1` entsprechen einfach der Summe aus dem Wert neben `_cons` und dem Koeffizienten neben `var1` multipliziert mit dem jeweiligen Wert für `var1`.[^50]
...
Vorhergesagte Werte werden mit $\widehat{var2}$  bezeichnet - das ^ steht dabei für "vorhergesagt":
...
[^50]: Die Option `noheader` macht den Output von `reg` etwas übersichtlicher.
...
Wir sehen uns den (möglichen) Zusammenhang zwischen diesen beiden Merkmalen an. Dazu definieren die negativen Ausprägungen sowie die Kategorien "noch Schüler" und "anderer Schulabschluss" als Missing, um eine klare ordinale Rangfolge für `educ` zu erhalten (je höher,desto höher der Bildungsabschluss):  
...
$$ Cramer's\,\,\upsilon = \sqrt{\frac{\chi^2}{n*\,min(k-1,m-1)}}=\sqrt{\frac{90.1737}{3466*1}} = 0.1613$$
  
Dieser Wert für Cramer's $\upsilon$ legt einen geringen Zusammenhang nahe.
...
(@) Laden Sie den **kumulierten** Allbusdatensatz (`Allbus_1980-2018.dta`) in Stata und wählen Sie die Befragten aus dem Jahr 2014 aus! Untersuchen Sie den Zusammenhang zwischen dem Alter der Befragten `age` und der Dauer des Fernsehkonsums (`lm02`) in Minuten!
    ...
    + Welche Fernsehdauer würden Sie für eine\*n 50-jährigen Befragten vorhersagen? Tipp[^055]
    + Welche Fernsehdauer würden Sie für eine\*n 51-jährigen Befragten vorhersagen?
    + Welche Fernsehdauer würden Sie für eine\*n 52-jährigen Befragten vorhersagen?
    + Vergleichen Sie diese drei Werte: was fällt Ihnen auf wenn Sie die

[^055]: Berechnen Sie ein [Regressionsmodell](#reg_pred)
...
Wie hoch ist nun der vorhergesagte Wert auf Basis der blauen Gerade? 
Die vohergesagten Werte aus `reg var2 var1` entsprechen einfach der Summe aus dem Wert neben `_cons` und dem Koeffizienten neben `var1` multipliziert mit dem jeweiligen Wert für `var1`.[^52]
...
[^52]: Die Option `noheader` macht den Output von `reg` etwas übersichtlicher.
...
ggplot(df, aes(x = var1, y = var2)) + geom_point(size = 3) + ggthemes::theme_stata() +
  ...
  theme(aspect.ratio = 1)
```
$\rightarrow$ *Wie groß ist die (einfache) Summe der Residuen für `pred_vorher`?*[^053]
