...
(@) Leben ältere Menschen eher in kleineren Orten? Berechnen Sie ein geeignetes Maß für den Zusammenhang zwischen `gkpol` und dem Alter der Befragten (`age`)! 
    + Berechnen Sie diesen Zusammenhang für das Jahr 2014! Laden Sie also den kumulierten Allbusdatensatz (`Allbus_1980-2018.dta`) in Stata und wählen Sie die Befragten aus dem Jahr 2014 aus.
...
(@) Häufig werden auch quasi-metrischen Skalen aus mehreren Variablen gebildet, z.B. wenn mehrere Indikatorfragen die Einstellung der Befragen zum Islam wiedergeben wie in `mm01` - `mm05`:
```{stata d_mm, echo =F}
set linesize 90
qui cd "D:\Studium\01_Oldenburg\Lehre\Datensaetze"
qui use  "Allbus_1980-2018.dta",clear
