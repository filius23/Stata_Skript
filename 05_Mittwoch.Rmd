...
```{r reg_korr,out.width = "100%", fig.height=6.5, fig.align="center" , echo = F,warning=F,message=F, dpi = 900}
eq1 <-  substitute(italic(bar(x))) # formel erstellen
eq2 <-  substitute(italic(bar(y))) # formel erstellen

set.seed(212221);tibble(x1 = 1:40) %>% mutate(e = runif(n(),-10,10),
                             y1 = 12 + 2.4*x1  + 0.00000012 * x1^2 - 0.000012 * x1^3 + 3*e,
                             y2 = 12 + 2.4*x1  + 0.00000012 * x1^2 - 0.000012 * x1^3 + 20*e,
                             y3 = 12 - 2.4*x1  + 0.00000012 * x1^2 - 0.000012 * x1^3 + 3*e,
                             y4 = 12 - 2.4*x1  + 0.00000012 * x1^2 - 0.000012 * x1^3 + 20*e
                             
                                   ) %>% 
  pivot_longer(cols=matches("^y"),names_to="var", values_to = "y") %>% 
  group_by(var) %>% 
  mutate(meany = mean(y),
         meanx = mean(x1),
         laby = ifelse(1:n() !=1,NA,1),
         labx = ifelse(1:n() !=1,NA,1)) %>% 
  nest() %>% mutate(corr = map(data,  function(df) cor(df$y,df$x1)) %>% unlist(.),
                                      reg = lapply(data, function(df) {
                                                       lm(y ~ x1, data = df) %>% broom::tidy() %>% 
                                                         mutate(estimate = round(estimate,3), 
                                                                lab = case_when(lead(estimate,1)>0 ~ "+",
                                                                                lead(estimate,1)<0 ~ "-",
                                                                                term == "x1" ~ "* x"),
                                                                f1 = paste(
                                                                  ifelse(term=="x1",str_remove(estimate,"-"),estimate),
                                                                  lab)) %>% 
                                                         pull(f1) %>% paste(.,collapse = " ")
                                            }) %>% unlist(.)) %>% 
  mutate(lab = paste0("Korrelation: ", round(corr,3), 
                      "\nRegression: ", reg)) %>% 
  unnest(data) %>% 
  ggplot(.,aes(x=x1,y=y,color = var)) + 
  geom_point(alpha = .5, stroke = .24) +
  geom_smooth(method = "lm", se= F, size = .6) +
  geom_vline(aes(xintercept = meanx), color = "#19D4EE", size = .25) +
  geom_hline(aes(yintercept = meany),color = "#2473D5", size = .25) +
  geom_label(aes(x = meanx,y=300,label =  ifelse(labx==1, as.character(as.expression(eq1)),NA) ), 
             label.size = .01, hjust = 0.5, color = prismatic::clr_darken("#19D4EE",.1), parse = T, size = 4.5) + 
  geom_label(aes(x = 0,y=meany,label =  ifelse(laby==1, as.character(as.expression(eq2)),NA) ) ,
            label.size = .01, hjust = 0.5, vjust=.8, color = prismatic::clr_darken("#19D4EE",.1), parse = T, size = 4.5) +
  facet_wrap(~lab,ncol = 2) +
  ggthemes::theme_stata(base_size = 9) +
  scale_color_manual(values=c("#29304E","#52609C","#575A69","#81869C")) +
  guides(color = F) +
  coord_cartesian(xlim = c(0.5,41)) +
  labs(x="x")+ 
...
Bei Bindungen in X o. Y Maximalwerte (-1; +1) nicht zu erreichen
...
```{r codebuch_cf01, echo = F,out.width = "80%",out.height="80%", fig.align="center"}
knitr::include_graphics("05_cdb_cf01.png")
```
...
```{r codebuch_px01, echo = F,out.width = "80%",out.height="80%", fig.align="center"}
knitr::include_graphics("05_cdb_px01.png")
```
...
<!--     + Berechnen Sie (mind.) ein passendes Zusammenhangsmaß, wenn Siese neue, kategorisierte Altersvariable anstelle von `age` verwenden! Ist der Zusammenhang als stark einzustufen? -->
