...
## Bootstrapping
Bootstrapping beruht auf der Idee, wiederholt Stichproben *B* aus der Stichprobe zu ziehen und aus dieser *B* die Parameter zu berechnen. Für das Bootstrapping ziehen wir wiederholt SP *mit Zurücklegen* der gleichen Größe wie die Ausgangs-SP, also hier n = `r length(a14m$hs16)` und berechnen anschließend das arith. Mittel für diese simulierte Stichprobe. Diesen Vorgang wiederholen wir x Male und berechnen für die so gesammelten Mittelwerte aus den simulierten Stichproben die Standardabweichung. Analog zur vorhin gezeigten parametrischen Methode können wir das KI dann als $mean\,\pm t * sd()$ berechnen:
