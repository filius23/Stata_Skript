...
Häufig möchten wir aber vielleicht nur eine bestimmte Information. Dafür ist `tabstat` eine gute Lösung. Mit `tabstat` können wir eine ganze Reihe statistischer Kennzahlen für metrische/kontinuierliche Merkmale berechnen. Die Syntax hat dabei immer das folgende Format: `tabstat age, s(mean median)`
...
Das funktioniert auch für alle `tabstat`-Befehle:

```{stata bytabstF, eval = F}
bys eastwest: tabstat age, s(cv)
```
...
ggplot(gdf, aes(x = pct_id, y = pct_di05)) + 
  ...
  geom_segment(data = gini_ann_df, aes(xend = .5, x = 0 , y = 0.5, yend = .5), size = .35, linetype = 2) + # gestrichelte Linie
...
Neben der Variante [`bys var:`](#bys_vgl) können wir auch mit Hilfe der `if`-Option Kennzahlen miteinander vergleichen:
