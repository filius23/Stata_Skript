...
(@) Erstellen Sie jeweils einen Datensatz für das Befragungsjahr 1994 und 2010 - `filter`n Sie also den kumulierten Datensatz jeweils einmal nach einem Jahr und legen Sie das Ergebnis in separaten Objekten ab, zB. `a94`.
(@) Betrachten Sie zunächst für 1994 die HH-Einkommensverteilungen (Variable `di05`) 
  + Denken Sie daran, die negativen Werte mit `NA` zu überschreiben!
  + Berechnen Sie das arithm. Mittel, den Median, das 25%- und 75%-Quartil sowie die Varianz zunächst für alle Fälle im Datensatz für 1994 - was sagen Ihnen die Werte jeweils?
...
(@) Betrachten Sie dann die HH-Einkommensverteilungen für 2010 (Variable `di05`) 
  + Denken Sie daran, die negativen Werte mit `NA` zu überschreiben!
  + Berechnen Sie jeweils für Ost und West das arithm. Mittel, den Median, das 25%- und 75%-Quartil sowie die Varianz zunächst für alle Fälle im Datensatz für 2010 - was sagen Ihnen die Werte jeweils?
(@) Vergleichen Sie die Werte aus 1994 und 2010. Welche Tendenz können Sie für die Ost-West-Unterschiede erkennen?
(@) Erstellen Sie jeweils einen Boxplot für Ost und West!
  ...
  + Was können Sie jeweils zur den Einkommensverteilungen sagen? 
  + Durch Anhängen des Arguments `+  coord_cartesian(ylim = c(min,max))` können Sie "zoomen" indem Sie statt `min` und `max` die Limits setzen.    
...
(@) Nutzen Sie den `Allbus2018.dta` und erstellen Sie eine Kontingenztabelle für `sex` und `gkpol`. Welche Merkmalskombination ist die häufigste? 
    + Denken Sie daran, fehlende Werte mit  `mvdecode` auszuschließen.
...
```{r dpi = 800, echo = F, out.height="70%", out.width="70%", fig.align="center"}
...
In Stata bekommen wir die Schiefe und Wölbung einer Verteilung mit `summary varname,detail` angezeigt.
