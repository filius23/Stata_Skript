...
(@) Laden Sie den **kumulierten** Allbusdatensatz (`Allbus_1980-2018.dta`).
    + Betrachten Sie dann die HH-Einkommensverteilungen für 2010 (Variable `di05`), behalten Sie also mit Hilfe von `keep` oder `drop` nur die Angaben aus dem Befragungsjahr 2010 im Speicher.
    + Denken Sie daran, die negativen Werte für `di05` mit `.` zu überschreiben!
...
(@) Erstellen Sie eine Häufigkeitstabelle für das Geburtsjahr der Befragten (`yborn`)!
    ...
    + Gibt es fehlende Werte, die Sie mit `mvdecode`/`replace` überschreiben sollten?
