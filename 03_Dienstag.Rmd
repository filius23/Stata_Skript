...
```
    ...
    + Hat die Variable fehlende Werte, die Sie ausschließen sollten? Falls ja, ersetzen Sie diese Werte mit `mvdecode` und lassen Sie sich die Tabelle erneut ausgeben. (Verwenden Sie `labelbook` oder sehen Sie im Codebuch nach).
    ...
    + Erstellen Sie nun eine Kontigenztabelle mit `gkpol` und `sex`. Welche Merkmalskombination ist die häufigste? 
